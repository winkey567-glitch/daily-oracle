// Daily Oracle — A2MCP Divination API
// POST /api/divine  →  I Ching hexagram reading via 3-coin method
// GET  /api/divine  →  same, for quick testing
...
// ============================================================
// Divination logic (3-coin method)
// ============================================================
function tossCoins() {
...
function castHexagram() {
  ...
  const hasChanged = primaryBinary !== changedBinary;
  const changedId = hasChanged ? parseInt(changedBinary, 2) + 1 : null;
  return {
    primaryId,
    primaryHex: HEXAGRAMS[primaryId],
    primaryBinary,
    changedId,
    changedHex: hasChanged ? HEXAGRAMS[changedId] : null,
    changingLines,
    lines,
...
function getUnicodeSymbol(id) {
  return String.fromCodePoint(0x4DC0 + id - 1);
}

// ============================================================
// HTTP Handler (Vercel Serverless Function)
// ============================================================
export default function handler(req, res) {
  ...
  try {
    ...
    const question = (req.body && req.body.question) || req.query.question || null;

    const response = {
      success: true,
      service: 'Daily Oracle - I Ching Divination',
      question: question,
      hexagram: {
        id: result.primaryId,
        name: result.primaryHex.name,
        en: result.primaryHex.en,
        symbol: getUnicodeSymbol(result.primaryId),
        binary: result.primaryBinary,
        fortune: result.primaryHex.fortune,
        level: result.primaryHex.level,
        judgment: result.primaryHex.judgment,
        image: result.primaryHex.image,
        reading: result.primaryHex.reading,
        tags: result.primaryHex.tags
      },
      changing_lines: result.changingLines,
      lines: result.lines.map(l => ({
        value: l,
        type: l === 6 ? 'old_yin' : l === 7 ? 'young_yang' : l === 8 ? 'young_yin' : 'old_yang'
      })),
      changed_hexagram: result.changedHex ? {
        id: result.changedId,
        name: result.changedHex.name,
        en: result.changedHex.en,
        symbol: getUnicodeSymbol(result.changedId),
        fortune: result.changedHex.fortune
      } : null,
